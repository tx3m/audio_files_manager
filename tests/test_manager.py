...
class TestAudioFileManager(unittest.TestCase):
    ...
    def test_temp_record_and_finalize(self):
        ...
        meta = self.manager.metadata_manager.metadata['btn1']
    ...
    def test_discard_temp_keeps_confirmed(self):
        ...
        self.manager.metadata_manager.metadata['btn2'] = {
            "name": "confirmed.wav",
        ...
        temp = self.manager.fs_manager.temp_dir / f"btn2_test_{int(time.time())}.wav"
    ...
    def test_assign_and_restore_default(self):
        ...
        self.assertTrue(self.manager.metadata_manager.metadata['btn3']['read_only'])

        self.manager.restore_default('btn3')
        restored = self.manager.metadata_manager.metadata['btn3']
    ...
    def test_set_read_only_toggle(self):
        self.manager.metadata_manager.metadata['btn4'] = {"read_only": False}
        self.manager.set_read_only('btn4', True)
        self.assertTrue(self.manager.metadata_manager.metadata['btn4']['read_only'])

    def test_get_info_and_listings(self):
        self.manager.metadata_manager.metadata['btn5'] = {"message_type": "greeting"}
    ...
    def test_finalize_blocked_for_readonly_logs_warning(self):
        button_id = 'btn6'
        self.manager.metadata_manager.metadata[button_id] = {"read_only": True}
        self.manager.metadata_manager.save()

        dummy_info = {
            "button_id": button_id,
            "temp_path": str(self.manager.fs_manager.temp_dir / "dummy.wav")
        }
        ...
        self.assertTrue(self.manager.metadata_manager.metadata[button_id]['read_only']) # check it wasn't overwritten

    def test_cleanup_removes_temp_dir(self):
        temp_dir_path = self.manager.fs_manager.temp_dir
